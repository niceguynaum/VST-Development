...
CSimpleHPF::CSimpleHPF()
{
	...
	//Sets the coefficient values to match the initialized slider settings
	//Calculates the new a0 values
	//clears out the z^-1 values
	//important for the equation a0 = a1 - 1.0 for high pass filtering where 0.0 <= a1 <= 0.49
	...
	//Cooking the volume from float values into logarithmic dB
	m_fVolume = pow(10.0, m_fVolumeControl / 20.0);
...
bool __stdcall CSimpleHPF::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
	...
	{
		// Handling the Right Channel since this is a catch for a stereo based effect.
		// Input sample is x(N)
...
bool __stdcall CSimpleHPF::userInterfaceChange(int nControlIndex)
{
	...
	switch(nControlIndex)
	{
		...
		{
			// save a1
			m_f_a1_left = m_fSlider_a1; //left first in case of mono select* 
			m_f_a1_right = m_fSlider_a1;
		...
		case 1:
		{
			//Volume Adjustments for the HPF
			m_fVolume = pow(10.0, m_fVolumeControl / 20.0);
			break;
		}
...
// **--0xDEA7--**
	...
	ui1->cControlName = "Volume Control";
